...
bool MapViewWindow::ProcessSDLEvent(const SDL_Event& event)
{
	...
	case SDL_MOUSEMOTION:
	{
		...
		if (mouseDown && inBounds)
		{
			...
			mousePoint = { event.button.x, event.button.y };
			m_worldView->CameraRot.x += mouseDiff.y / frameDelta;
			m_worldView->CameraRot.y += mouseDiff.x / frameDelta;
	...
	case SDL_MOUSEWHEEL:
	{
		m_worldView->CameraSpeed += (float)event.wheel.y / frameDelta * 10.0f;
...
bool MapViewWindow::Render()
{
	...
	{
		quitMenu = false;
		new MapSelectWindow(this, m_worldView->GetLocale());
	}
...
bool MapViewWindow::Update()
{
	...
	if (m_renderViewportFocused)
	{
		...
		else if (keyState[SDL_SCANCODE_W])
		{
			...
			m_worldView->CameraPos += direction / glm::vec3((float)frameDelta) * m_worldView->CameraSpeed;
		}
		...
		{
			m_worldView->CameraRot.y -= 0.1f / (float)frameDelta * m_worldView->CameraSpeed;
		}
		...
		{
			m_worldView->CameraPos.y -= 1.0f / (float)frameDelta * m_worldView->CameraSpeed;
		}
